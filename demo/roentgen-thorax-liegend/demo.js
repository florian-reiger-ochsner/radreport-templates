// =============================================================================
// Demo-Interaktivität für "Röntgen Thorax a.p. liegend / Intensiv" (v1.0)
//
...
//
// Besonderheit ICU / portable chest: Die Region "Lage-/Device-Kontrolle" ist
// FÜHREND (erste Region, mit Device-Stack im Detail). Zeichen-Vokabular auf
// a.p.-Rückenlage abgestimmt (keine CTR-Wertung, Liegend-Zeichen Erguss/PTX,
// pulmonale Stauung als Leitbefund in der Lungen-Region).
// =============================================================================
...
const ORDER = ['devices', 'lunge', 'pleura', 'herzmediastinum', 'skelett'];
...
const REG = {
  devices: {
    crit: false,
    critWhen() { return devCrit(); },
    neg: 'Lage-/Device-Kontrolle: Keine einliegenden Katheter, Sonden, Tuben oder sonstiges Fremdmaterial.',
    build(st) {
      if (st === 'ob') return this.neg;
      const items = getDevItems();
      if (!items.length) return 'Lage-/Device-Kontrolle: Befund gewählt – bitte Device ergänzen.';
      const t = items.map(i => { let s = i.typ; if (i.lage) s += ` (${i.lage})`; s += ` – ${i.burt}`; if (i.ft) s += `, ${i.ft}`; return s; }).join('; ');
      return 'Lage-/Device-Kontrolle: ' + t + '.';
    }
  },
  lunge: {
    crit: false,
    neg: 'Lunge: Soweit bei a.p.-Liegendaufnahme beurteilbar seitengleich belüftet. Keine pulmonale Stauung, kein Infiltrat, keine flächige Transparenzminderung.',
    build(st) {
      if (st === 'ob') return this.neg;
      const parts = [];
      const stau = gv('lunge_stau');
      if (stau) parts.push(`${stau.charAt(0).toUpperCase() + stau.slice(1)} pulmonale Stauung`);
      getParItems().forEach(i => {
        let s = i.de;
        if (i.seite && i.lok) s += ` ${i.seite} (${i.lok})`; else if (i.seite) s += ` ${i.seite}`; else if (i.lok) s += ` (${i.lok})`;
        if (i.ft) s += `, ${i.ft}`;
        parts.push(s);
      });
      if (!parts.length) return 'Lunge: Pathologischer Befund – bitte Stauung/Typ wählen.';
  ...
  pleura: {
    ...
    neg: 'Pleura: Kein Nachweis eines Pneumothorax (soweit bei Liegendaufnahme beurteilbar). Kein größerer Pleuraerguss.',
    build(st) {
      if (st === 'ob') return this.neg;
  ...
  herzmediastinum: {
    crit: false,
    neg: 'Herz und Mediastinum: Herzsilhouette bei a.p.-Liegendaufnahme nicht sicher beurteilbar, orientierend nicht wesentlich verbreitert. Mediastinum nicht pathologisch verbreitert. Hili bds. orientierend regelrecht.',
    build(st) {
      if (st === 'ob') return this.neg;
      const gr = gv('herz_gr');
      const ven = [gc('gefstiel') ? 'Gefäßstiel verbreitert' : null, gc('vcs_stau') ? 'obere Einflussstauung/-umverteilung' : null].filter(Boolean).join(', ');
      const med = [gc('md_verb') ? 'verbreitert (Cave: liegend projektionsbedingt)' : null, gc('md_rf') ? 'Raumforderung' : null, gc('md_trach') ? 'Trachealdeviation' : null, gc('md_shift') ? 'Mediastinalshift kontralateral' : null].filter(Boolean).join(', ');
      const hs = gv('hil_seite');
      const hil = [gc('hil_vergr') ? 'vergrößert' : null, gc('hil_versch') ? 'verschattet' : null].filter(Boolean).join(', ');
      let s = `Herz und Mediastinum: Herzsilhouette ${gr || 'orientierend beurteilt'}`;
      if (ven) s += `, ${ven}`;
      s += '.';
      s += med ? ` Mediastinum: ${med}.` : ' Mediastinum nicht pathologisch verbreitert.';
      if (hil) s += ` Hilus${hs ? ' ' + hs : ''}: ${hil}.`; else s += ' Hili bds. orientierend regelrecht.';
      return s;
    }
  },
  skelett: {
    ...
    neg: 'Skelett und Weichteile: Miterfasstes Skelett und Weichteile der Thoraxwand orientierend unauffällig.',
    build(st) {
      if (st === 'ob') return this.neg;
...
(function buildChrome() {
  ...
  pane.querySelector('.rr-title-rule').insertAdjacentHTML('afterend', `
    <div class="rr-tx-macro-bar">
      <div class="rr-tx-macro-txt"><strong>Additives Modell:</strong> Keine Region ist vorbelegt. Du <strong>attestierst</strong> (o.&nbsp;B.) oder <strong>fügst Pathologie hinzu</strong> (Befund). Der Lage-/Device-Block führt (ICU / portable chest). Der Makro setzt alle offenen Regionen in einem Akt auf o.&nbsp;B.</div>
  ...
  pane.insertAdjacentHTML('beforeend', `
    ...
    <div class="rr-tx-loinc-ovr">
      <label>
        ...
        <select id="loinc_override">
          <option value="36589-0|Portable XR Chest AP single view">36589-0 · Portable XR Chest AP single view — Default (mobil/liegend)</option>
          <option value="36572-6|XR Chest AP">36572-6 · XR Chest AP</option>
          <option value="36554-4|XR Chest Single view">36554-4 · XR Chest Single view</option>
        </select></label>
    </div>
    ...
    <div class="rr-tx-legend"><span class="rr-tx-rl">RadLex</span> RID-Codes auf allen Optionen · <span class="rr-tx-rl">LOINC</span> 36589-0 (Default portable AP, Override möglich) im DiagnosticReport · <span class="rr-tx-rl">FHIR</span> R4 – NEG/POS-Interpretation als Verification Floor</div>
  `);
  ...
  app.insertAdjacentHTML('beforeend', `
    <aside class="rr-preview-pane">
      ...
      <div class="rr-tx-rcard">
        <div class="rr-tx-rcard-header"><div class="rr-tx-rcard-title">Röntgen Thorax a.p. liegend</div><div class="rr-tx-rcard-sub" id="prev-sub">–</div></div>
        ...
        <div class="rr-tx-stamp"><span>HJK-MRRT-ROE-THORAX-LIEGEND-v1.0</span><span id="prev-date"></span></div>
      </div>
...
function getParItems() {
  ...
  document.querySelectorAll('.rr-tx-par-item').forEach(el => {
    ...
    items.push({ rid: o?.dataset.radlex || '', de: o?.dataset.de || '', en: o?.dataset.en || '', seite: gv(`par-seite-${id}`), lok: gv(`par-lok-${id}`), ft: gv(`par-ft-${id}`) });
  });
  return items.filter(i => i.rid);
}

// -----------------------------------------------------------------------------
// 7) Device-Stack (klont die kanonische Erst-Instanz) – führender Block
// -----------------------------------------------------------------------------
function addDevItem() {
...
function buildTechnik() {
  const projs = [];
  if (gc('p_ap')) projs.push('a.p. liegend'); if (gc('p_halb')) projs.push('a.p. halb aufrecht');
  if (gc('p_mobil')) projs.push('mobile Bettaufnahme'); if (gc('p_insp')) projs.push('eingeschränkte Inspiration');
  const q = gv('qual') || 'für Liegendaufnahme diagnostisch ausreichend';
  const f = gv('frage') === '__ft__' ? gv('frage_ft') : gv('frage');
  let t = `Röntgen Thorax, ${projs.join(' + ') || 'a.p. liegend'}. Aufnahmequalität ${q}.`;
...
function buildBeurteilung() {
  ...
  pos.forEach(r => {
    if (r.key === 'devices') {
      getDevItems().forEach(d => { if (/dringlich/i.test(d.burt)) parts.push(`${d.typ}: Fehllage – dringliche Korrektur erforderlich.`); else if (/Fehllage/i.test(d.burt)) parts.push(`${d.typ}: Fehllage – Korrektur empfohlen.`); else if (/kontrollbedürftig/i.test(d.burt)) parts.push(`${d.typ}: Lage kontrollbedürftig.`); });
    }
    else if (r.key === 'lunge') {
      const stau = gv('lunge_stau'); if (stau) parts.push(`${stau.charAt(0).toUpperCase() + stau.slice(1)} pulmonale Stauung.`);
      getParItems().forEach(i => { let s = i.de; if (i.seite) s += ` ${i.seite}`; if (i.lok) s += ` (${i.lok})`; parts.push(s + '.'); });
    }
  ...
  if (!parts.length) { if (nUnassessed() === 0) return 'Röntgen Thorax a.p. liegend ohne pathologischen Befund; einliegendes Fremdmaterial regelrecht platziert.'; return ''; }
...
function hasCrit() { return Object.keys(REG).some(key => regStatus(key) === 'bef' && (REG[key].crit || (REG[key].critWhen && REG[key].critWhen()))); }
...
function update() {
  ...
  if (gc('p_ap')) projs.push('a.p.'); if (gc('p_halb')) projs.push('½ aufr.'); if (gc('p_mobil')) projs.push('mobil'); if (gc('p_insp')) projs.push('Insp.↓');
  document.getElementById('prev-sub').textContent = (projs.join(' + ') || 'a.p. liegend') + (f ? ' · ' + f : '');
...
function buildFhir() {
  ...
  oREG.forEach(r => {
    ...
    if (r.key === 'devices') {
      ...
      items.forEach((d, k) => { let txt = d.typ + (d.lage ? ' (' + d.lage + ')' : '') + ' – ' + d.burt + (d.ft ? ', ' + d.ft : ''); push(d.rid, d.en, txt, false, `dev-${k + 1}`); });
    } else if (r.key === 'lunge') {
      const stau = gv('lunge_stau'); if (stau) push(elRid('lunge_stau'), elEn('lunge_stau'), stau + ' pulmonale Stauung', false, 'lunge-stau');
      const items = getParItems();
      if (!items.length && !stau) { push(regRid(r.key), regEn(r.key), r.build('bef'), false, r.key); }
      ...
      if (gc('vcs_stau')) push(elRid('vcs_stau'), elEn('vcs_stau'), 'obere Einflussstauung/-umverteilung', false, 'vcs-stau');
  ...
  return JSON.stringify({ resourceType: 'Bundle', type: 'document', timestamp: now, meta: { tag: [{ system: 'http://hjk.wien/fhir/CodeSystem/radiology-templates', code: 'radlex-coded' }] }, entry: [{ fullUrl: 'urn:uuid:thorax-liegend-report', resource: dr }, ...obsArr] }, null, 2);
}
...
function resetAll() {
  ...
  const ap = document.getElementById('p_ap'); if (ap) ap.checked = true;
